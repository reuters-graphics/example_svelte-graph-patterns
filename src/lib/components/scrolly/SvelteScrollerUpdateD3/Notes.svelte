...
<ul>
	<li>
		In the Graph Svelte component, we pass props from the Scroller, 
...
<ul>
	<li>
		The takeaway is: <span class='accent'>use reactive statements or declarations if referring to Scrolly variables to ensure enclosed logic updates correctly</span>.
More specifically: 
	</li>
	<li>
	...
	<li>
		Set up the Graph from <code>onMount</code>, passing it the container, (initial) props and (initial) data.
	</li>
	<li>
		...
		to the given step, e.g. <code>$: graph.props(props).draw()</code>
	</li>
	<li>
...
<ul>
	...
	<li>
		The background container has height of 100vh (red lines) but the content can have less, 
	...
	<li>
		The scrolly becomes sticky when the top of the background container hits the top of the viewport. 
	...
	<li>
		The scroller threshold is 0.5, meaning that for each step, it becomes active when 
	...
	<li>
		If you don't like having the 0th step at the top of the scroller container, 
