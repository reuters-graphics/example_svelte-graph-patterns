<p class='summary'>
	Svelte Scroller with a D3 chart using GSAP to transition on ScrollTrigger-bsased steps
</p>
...
<ul>
	<li>
		...
		passed in as a prop from Svelte Scroller, not write any conditional on index 
		code for the graph state inside of Svelte. 
		Instead, we give each step DOM element (here a div) a unique id and use this 
		to <span class='accent'>target the step entirely from the vanialla JS-based GSAP ScrollTrigger code</span>. 
	...
	<li>
		...
		e.g. <code>stepOneLogic()</code> as and then call all of them as callbacks to <code>onToggle</code> 
		...
		Note that here each step will become active and therefore display the graph state for that step 
	...
	<li>
		In the Svelte code, we wrap the graph-drawing logc inside a <span class='accent'>reactive declaration</span>,
		which also checks that the DOM element into which we are injecting the graph exsits. 
